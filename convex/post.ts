...
export const createPost = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const blogArticleId = await ctx.db.insert("posts", {
      title: args.title,
      body: args.body,
      authorId: user._id,
...
export const searchPosts = query({
  ...
  handler: async (ctx, args) => {
    ...
    const seen = new Set(); // to make sure we don't return duplicates

    const pushDocs = async (docs : Array<Doc<"posts">>) => {
      for (const doc of docs) {
        if (seen.has(doc._id.toString())) continue;
        seen.add(doc._id.toString());
        const url = doc.imageStorageId ? await ctx.storage.getUrl(doc.imageStorageId) : null;
        results.push({ 
            _id: doc._id,
            imageUrl: url,
            title: doc.title,
    ...
    const titleMatches = await ctx.db.query("posts")
      .withSearchIndex("title",(q) => q.search('title', args.term))
        .take(limit)  
